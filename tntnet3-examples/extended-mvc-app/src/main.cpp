...
int main(int argc, char* argv[])
{
  try
  {
    ...
    // read option "-c <configfile>" with default value "MyApp.xml":
    cxxtools::Arg<const char*> configfilename(argc, argv, 'c', "MyApp.xml");
    ...
    // Read static resources compiled into the binary
      app.mapUrl("^/(.*)", "public")
      .setPathInfo("src/public/$1");

    // Actions
    //
      app.mapUrl("^/(.*)\\.action$", "src/actionmain")
       .setArg("next", "$1");
    ...
    // with the view component after the controller
       app.mapUrl("^/$", "src/api/controllers/index")  // controller for index page
       .setHttpReturn(DECLINED);

       app.mapUrl("^/(.*)$", "src/api/controllers/$1")  // controller for pages routed through webmain
       .setHttpReturn(DECLINED);
    ...
    // View
    app.mapUrl("^/$", "src/webmain")  // index page
       .setArg("next", "index");
    app.mapUrl("^/(.*)$", "src/webmain")
       .setArg("next", "$1");
